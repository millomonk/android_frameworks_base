...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public int finishPostLayoutPolicyLw() {
        ...
        if (mStatusBar != null) {
            ...
            if (expandedDesktopHidesStatusBar() && !isStatusBarShownTemporarily()) {
                if (DEBUG_LAYOUT) Log.v(TAG, "Hiding status bar: expanded desktop enabled");
                if (mStatusBar.hideLw(true)) {
                    changes |= FINISH_LAYOUT_REDO_LAYOUT;
                    triggerStatusBarCollapse();
                }
                ...
                // case though.
                if ((topIsFullscreen || expandedDesktopHidesStatusBar()) && !isStatusBarShownTemporarily()) {
            ...
            mHandler.post(new Runnable() {
                public void run() {
                    try {
                        IStatusBarService statusbar = getStatusBarService();
                        if (statusbar != null) {
                            statusbar.setStatusBarHiddenState(mTopIsFullscreen);
                        }
                    } catch (RemoteException e) {
                        // re-acquire status bar service next time it is needed.
                        mStatusBarService = null;
    ...
    private void triggerStatusBarCollapse() {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                try {
